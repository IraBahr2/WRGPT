...
class HandCollector:
    ...
    def parse_status_page(self) -> List[TableStatus]:
        ...
        try:
            ...
            for row in table.find_all('tr')[1:]:  # Skip header row
                ...
                if status != "Broken":
                    tables.append(TableStatus(table_id=table_id, current_hand=hand_num))
    ...
    def collect_hands_for_table(self, table_id: str, up_to_hand: int) -> None:
        ...
        self.logger.info(f"Collecting hands for table {table_id} from hand {start_hand} up to hand {up_to_hand - 1}")
        
        for hand_num in range(start_hand, up_to_hand):  # Process hands up to but not including current
            try:
    ...
    def collect_all_hands(self) -> None:
        ...
        for idx, table in enumerate(tables, 1):
            self.logger.info(f"Processing table {table.table_id} ({idx}/{total_tables}), current hand: {table.current_hand}")
            self.collect_hands_for_table(table.table_id, table.current_hand)
